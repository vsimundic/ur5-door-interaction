...
#define RVLQLISTARRAY_ADD_ENTRY(ListArray, i, pEntry) \
	{                                                 \
		*(ListArray[i].ppNext) = pEntry;              \
		ListArray[i].ppNext = &(pEntry->pNext);       \
		pEntry->pNext = NULL;                         \
	}

#define RVLQLISTARRAY_ADD_ENTRY2(ListArray, i, pEntry) \
	{                                                  \
		pEntry->pPtrToThis = ListArray[i].ppNext;      \
		*(ListArray[i].ppNext) = pEntry;               \
		ListArray[i].ppNext = &(pEntry->pNext);        \
		pEntry->pNext = NULL;                          \
	}

#define RVLQLIST_ADD_ENTRY(pList, pEntry) \
	{                                     \
		*(pList->ppNext) = pEntry;        \
		pList->ppNext = &(pEntry->pNext); \
		pEntry->pNext = NULL;             \
	}

#define RVLQLIST_ADD_ENTRY2(pList, pEntry)  \
	{                                       \
		pEntry->pPtrToThis = pList->ppNext; \
		*(pList->ppNext) = pEntry;          \
		pList->ppNext = &(pEntry->pNext);   \
		pEntry->pNext = NULL;               \
	}

#define RVLQLIST_INIT(pList)              \
	{                                     \
		pList->pFirst = NULL;             \
		pList->ppNext = &(pList->pFirst); \
	}

#define RVLQLIST_INIT2(pList, pList2)      \
	{                                      \
		pList->pFirst = NULL;              \
		pList->ppNext = &(pList2->pFirst); \
	}

#define RVLQLIST_INSERT_ENTRY(pList, pEntry1, pEntry2, pNewEntry) \
	{                                                             \
		if (pList->pFirst == pEntry2)                             \
		{                                                         \
			pList->pFirst = pNewEntry;                            \
			pNewEntry->pNext = pEntry2;                           \
		}                                                         \
		else                                                      \
		{                                                         \
			pEntry1->pNext = pNewEntry;                           \
			pNewEntry->pNext = pEntry2;                           \
		}                                                         \
	}

#define RVLQLIST_INSERT_ENTRY2(ppEntry, pNewEntry) \
	{                                              \
...
#define RVLQLIST_INSERT_ENTRY2_(ppEntry, pNewEntry)       \
	{                                                     \
		...
		if (*ppEntry)                                     \
			(*ppEntry)->pPtrToThis = &(pNewEntry->pNext); \
		pNewEntry->pNext = (*ppEntry);                    \
		*ppEntry = pNewEntry;                             \
	}

#define RVLQLIST_REMOVE_ENTRY(pList, pEntry, ppEntry) \
	{                                                 \
		if (pEntry->pNext == NULL)                    \
		{                                             \
			pList->ppNext = ppEntry;                  \
			*ppEntry = NULL;                          \
		}                                             \
		else                                          \
		{                                             \
...
#define RVLQLIST_GET_N_ENTRIES(pList, type, n)                                \
	{                                                                         \
		n = 0;                                                                \
...
#define RVLQLIST_INIT_(List)          \
	{                                 \
		List.pFirst = NULL;           \
		List.ppNext = &(List.pFirst); \
	}
#define RVLQLIST_ADD_ENTRY_(List, pEntry) \
	{                                     \
		*(List.ppNext) = pEntry;          \
		List.ppNext = &(pEntry->pNext);   \
		pEntry->pNext = NULL;             \
	}
#define RVLQLIST_ARRAY_INIT(Array, pQList) \
	{                                      \
		for (int i = 0; i < Array.n; i++)  \
		{                                  \
			pQList = Array.Element + i;    \
			RVLQLIST_INIT(pQList);         \
		}                                  \
	}
#define RVLQLIST_ARRAY_INIT2(pArray, Attribute, QListType)              \
	{                                                                   \
		for (int i = 0; i < pArray->n; i++)                             \
		{                                                               \
			QList<QListType> *pQList = &(pArray->Element[i].Attribute); \
			RVLQLIST_INIT(pQList)                                       \
		}                                                               \
	}
...
#define RVLQLIST_GET_NEXT_CIRCULAR(pList, pElement) \
	{                                               \
