...
{\
	Tgt[n*i+j] = Src[0]; Tgt[n*i+j+1] = Src[1]; Tgt[n*i+j+2] = Src[2];\
	Tgt[n*(i+1)+j] = Src[3]; Tgt[n*(i+1)+j+1] = Src[4]; Tgt[n*(i+1)+j+2] = Src[5];\
	Tgt[n*(i+2)+j] = Src[6]; Tgt[n*(i+2)+j+1] = Src[7]; Tgt[n*(i+2)+j+2] = Src[8];\
}
// Tgt(i:i+2, j:j+2) = Src', (Rows of Tgt have n elements)
#define RVLCOPY3BLOCKTTOMX(Src, Tgt, i, j, n)\
{\
	Tgt[n*i+j] = Src[0]; Tgt[n*i+j+1] = Src[3]; Tgt[n*i+j+2] = Src[6];\
	Tgt[n*(i+1)+j] = Src[1]; Tgt[n*(i+1)+j+1] = Src[4]; Tgt[n*(i+1)+j+2] = Src[7];\
	Tgt[n*(i+2)+j] = Src[2]; Tgt[n*(i+2)+j+1] = Src[5]; Tgt[n*(i+2)+j+2] = Src[8];\
}
// Z = X(3x3) + Y(3x3)
...
// Tgt = diag(a) * Src
#define RVLSCALE3VECTOR3(Src, a, Tgt)	{Tgt[0] = Src[0] * a[0]; Tgt[1] = Src[1] * a[1]; Tgt[2] = Src[2] * a[2];}
// Tgt = Src(3x3) * a
...
namespace RVL
{
	...
	template <typename Type> struct Sphere
	{
		Type center[3];
		Type radius;
	};
...
#define RVLCREATE3DTRANSF(R, t, T)\
		...
		T *P)
	{
		...
		P_ = P + 0 * 3; RVLSET3VECTOR(P_, pBox->minx, pBox->miny, pBox->minz);
		P_ = P + 1 * 3; RVLSET3VECTOR(P_, pBox->maxx, pBox->miny, pBox->minz);
		P_ = P + 2 * 3; RVLSET3VECTOR(P_, pBox->minx, pBox->maxy, pBox->minz);
		P_ = P + 3 * 3; RVLSET3VECTOR(P_, pBox->maxx, pBox->maxy, pBox->minz);
		P_ = P + 4 * 3; RVLSET3VECTOR(P_, pBox->minx, pBox->miny, pBox->maxz);
		P_ = P + 5 * 3; RVLSET3VECTOR(P_, pBox->maxx, pBox->miny, pBox->maxz);
		P_ = P + 6 * 3; RVLSET3VECTOR(P_, pBox->minx, pBox->maxy, pBox->maxz);
		P_ = P + 7 * 3; RVLSET3VECTOR(P_, pBox->maxx, pBox->maxy, pBox->maxz);
	}
