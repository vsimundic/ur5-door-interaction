...
void VNClassifier::Create(char *cfgFileName)
{
	...
	if (problem == RVLRECOGNITION_PROBLEM_SHAPE_INSTANCE_DETECTION && mode == RVLRECOGNITION_MODE_RECOGNITION)
	{
		...
		printf("Loading CTI descriptors from the database...");
	...
	PSGM_::CreateTemplateLookUpTable(sampledUnitSphere, 200, convexTemplateLUT);

	printf("completed.\n");

	// Load CTI descriptor mapper data.

	if(PSGM_::LoadCTIDescriptorMapping("..\\CTIDescriptorMapper.dat", CTIDescriptorMapingData))
	// RVL2020
	{
...
void VNClassifier::CreateSuperSegments(int minClusterSize)
{
	...
	do
	{
		if (pSuperSegment)
		{
			iFirstSegment = (pSuperSegment->segments.n < maxNObjectSegments ?
				pSuperSegment->segments.Element[pSuperSegment->segments.n - 1] + 1 : pObjects->objectArray.n);
...
void VNClassifier::DisplaySurfelMask(VN_::SurfelMask surfelMask)
{
...
void VNClassifier::GreedyInterpretation2()
{
	...
	for (i = 0; i < level3Hypotheses.n; i++)
	{
		...
		if (bUseColor)
		{
			...
			if (pHypothesis->matchedPtsPercentage < greedyInterpretationMatchedPercentageThr || pHypothesis->score > 0.0f)
				continue;
...
{	
	...
	if (bUseColor)
	{
		...
		pSurfels->CalculateSurfelsColorHistograms(pShapeInstanceDetection->pMesh, RVLColorDescriptor::ColorSpaceList::HSV, false, binsize, true, chFilterThr, true);
	}
	...
	for (iModel = 0; iModel < pShapeInstanceDetection->activeModels.n; iModel++)
	{
		...
		nLevel1Hypotheses += HSpace.Prune3DPoses(CTIMatchRedundantPoseOrientThr);
			...
			HSpace.CopyData(pHypothesisList, pMem, sizeof(VN_::Hypothesis2));
...
//sort hypotheses for every model
	...
	if (pShapeInstanceDetection->pECCVGT->GT.Element)
	{
		...
		//float *PMCHMem = new float[3 * maxnCHVertices];	// RVL2020
			...
			if ((pHypothesis->flags & RVLVN_HYPOTHESIS2_FLAG_LEVEL1) || bAllHypotheses)
			{
				...
				{	
					...
					{						
						...
						{
							GTHypothesis[iGT].pHypothesis = pHypothesis;	// RVL2020
							//GTHypothesis[iGT].pHypothesis = NULL;
	...
	//for (iClass = 0; iClass < classArray.n; iClass++)	// (ARP3D.TR3.9: Algorithm 2, line 16)
	{
		...
		for (iHypothesis = 0; iHypothesis < alignment.CTIDescMap.nCTI; iHypothesis++)
		{
			...
			if (bPrimitiveLayer && iClass == 0)
			{
				...
				if (J__[0] < 0.0f) J__[0] = -J__[0];
				if (J__[1] < 0.0f) J__[1] = -J__[1];
				if (J__[2] < 0.0f) J__[2] = -J__[2];
			}
			...
			if (iR_ >= 0)	// if there are valid alignment proposals
			{
				...
				//for (iHypothesis = 0; iHypothesis < nHypotheses; iHypothesis++)
				{
					...
					for (j = 0; j < convexHull.iVisibleFaces.n; j++)
					{
						...
						RVLVN_GET_CLOSEST_CONVEX_TEMPLATE_ELEMENT(convexTemplateLUT, uS, RMS_, uM, i_, j_, k_, iConvexTempleteElement);
	...
	Array<int> *piModelNormalArray)
{
	...
	for (k = 0; k < nIterations; k++)
	{
		...
		for (j = 0; j < pConvexHull->iVisibleFaces.n; j++)
		{
			...
			RVLVN_GET_CLOSEST_CONVEX_TEMPLATE_ELEMENT(convexTemplateLUT, uS, R, uM, i_, j_, k_, iCorrespondence);
...
template <typename T> struct SortIndexListEntry
{
	int idx;
	T cost;
