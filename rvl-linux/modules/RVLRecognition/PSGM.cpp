	...
	int iSurfel)
{
	...
	for (i = 0; i < pCluster->iSurfelArray.n; i++)
	{
		...
		if (bSurfelUncertainty)
			e = (pSurfel_->flags & RVLSURFEL_FLAG_RF ? pSurfels->Distance(pSurfel_, pVertex->P, true) : 
				RVLDOTPRODUCT3(pSurfel_->N, pVertex->P) - pSurfel_->d);
		else
	...
	int iFirstVertex)
{
	...
	for (iVertex_ = 0; iVertex_ < pCluster->iVertexArray.n; iVertex_++)
	{
		...
		if (bSurfelUncertainty)
			e = (pSurfel->flags & RVLSURFEL_FLAG_RF ? pSurfels->Distance(pSurfel, pVertex->P, true) :
				RVLDOTPRODUCT3(pSurfel->N, pVertex->P) - pSurfel->d);
		else
...
void PSGM::LoadModelMeshDB(char *modelSequenceFileName, std::map<int, vtkSmartPointer<vtkPolyData>> *vtkModelDB, bool bDecimate, float decimatePercent)
{
	...
	printf("Initializing dbLoader with %s...\n", modelsInDataBase);

	dbLoader.Init(modelsInDataBase);

	printf("dbLoader is initialized.\n");
	...
	while (modelsLoader.GetNext(modelFilePath, modelFileName))
	{
		...
		if (bUseColor)
		{
			...
			cdModelDB.at(modelID)->InsertArrayIntoHistogram(colorpix.data, noPts, true);
...
float PSGM::HypothesisEvaluationIP2(
	...
	RECOG::PSGM_::SceneFittingError *errorRecord)
{
	float maxe2 = maxe * maxe;
...
void PSGM_::CreateTemplateLookUpTable(
	Array2D<float> A,
	int resolution,
		...
		for (int i = iFirst; i < iFirst + n; i++) //for all non-referent models
	{
		...
		for (int k = 0; k < m_i; k++) //for all CTI-s in current model
		{
			...
			for (int di = 0; di < 66; di++)
			{
				...
				{
					if (di == 0) maxd = abs(D(di, k) + D(di + 33, k));
