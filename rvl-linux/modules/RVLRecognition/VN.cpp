	...
	)
{
	...
	while (pTorus)
	{
		...
		for (int iRing = 0; iRing < pTorus->ringArray.n; iRing++){ //prodi kroz sve prstene torusa
			...
			if (pRingS != NULL){
				for (int iEdge = 0; iEdge < pRingS->iEdgeArray.n; iEdge++){ //prodi kroz sve edgeve prstena
					...
					switch (pRingS->iBeta){
					case 0:
						ringColorsList[torusID]->InsertNextTupleValue(one_red);
						break;
					case 1:
						ringColorsList[torusID]->InsertNextTupleValue(two_green);
						break;
					case 2:
						ringColorsList[torusID]->InsertNextTupleValue(three_blue);
						break;
					case 3:
						ringColorsList[torusID]->InsertNextTupleValue(four_yellow);
						break;
					case 4:
						ringColorsList[torusID]->InsertNextTupleValue(five_magenta);
						break;
					case 5:
						ringColorsList[torusID]->InsertNextTupleValue(six_orange);
						break;
					case 6:
						ringColorsList[torusID]->InsertNextTupleValue(seven_cyan);
						break;
					}
				}
			}
		...
		ringEdgesList[torusID]->InsertNextCell(edge_helper);
		ringColorsList[torusID]->InsertNextTupleValue(axis_white);
