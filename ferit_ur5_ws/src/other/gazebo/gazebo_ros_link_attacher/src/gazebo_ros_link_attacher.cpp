...
namespace gazebo
{
  ...
  bool GazeboRosLinkAttacher::attach(std::string model1, std::string link1,
                                     ...
                                     double spring_stiffness, double damping)
  {
    ...
    // Added SPRING-DAMPER MECHANISM
    j.joint->SetParam("spring_stiffness", 0, spring_stiffness);
    j.joint->SetParam("damping", 0, damping);
    ROS_INFO_STREAM("Attach with spring-damper: stiffness=" << spring_stiffness
                                                            << ", damping=" << damping);
    j.joint->Init();
  ...
  {
    ROS_INFO_STREAM("Received request to attach with spring-damper: model1="
                    << req.model_name_1 << ", link1=" << req.link_name_1
                    << ", model2=" << req.model_name_2 << ", link2=" << req.link_name_2
                    << ", stiffness=" << req.spring_stiffness
                    << ", damping=" << req.damping);

    if (!this->attach(req.model_name_1, req.link_name_1,
                      req.model_name_2, req.link_name_2,
                      req.spring_stiffness, req.damping))
    {
