...
class FSMState(Enum):
    ...
    FRONT_SURFACE_TOUCHES  = "FRONT_SURFACE_TOUCHES"
    EXECUTE_APPROACH_PATH  = "EXECUTE_APPROACH_PATH"
    EXECUTE_INSERTION_PATH = "EXECUTE_INSERTION_PATH"
    EXECUTE_OPENING_PATH   = "EXECUTE_OPENING_PATH"
    EXIT                   = "EXIT"
...
class DoorReplanningFSM:
    ...
    def plan_trajectory_multi_contact(self, num_traj_pts):
        # Multi-contact planning via path2, wrapped to return the same (waypoint_trajectories, trajectories)
        # structure as the single-contact planner.
        ...
        empty_trajectories = np.empty((0, num_traj_pts + 2, 6), dtype=np.float32)
        ...
        root = Waypoint(q[0], T_6_0_array[0])
        node = root
        for i in range(1, q.shape[0]):
            node = node.add_child(q[i], T_6_0_array[i])

        waypoint_trajectories = [root]
        trajectories = q[np.newaxis, ...].astype(np.float32)
        rospy.loginfo(f"[FSM] Multi-contact path found: {q.shape[0]} waypoints.")
        return waypoint_trajectories, trajectories
    ...
    def plan_trajectory(self, num_traj_pts=10, height_scale=0.8):
        ...
        self.waypoint_trajectories, self.trajectories = \
            self.plan_trajectory_multi_contact(num_traj_pts)
    ...
    def run(self):
        ...
        for IS_OFFLINE in is_offline_mode_list:
            ...
            if IS_OFFLINE:
                ...
                num_trajectories = 3
                # self.state = FSMState.FRONT_SURFACE_TOUCHES
                self.state = FSMState.PLAN_TRAJECTORY
                is_plan_traj = True # debug purpose
                # self.state = FSMState.EXECUTE_APPROACH_PATH # debug purpose
            ...
            self.selected_poses = self.gt_poses
            self.state = FSMState.PLAN_TRAJECTORY  if is_plan_traj else FSMState.FRONT_SURFACE_TOUCHES
            ...
            for i_scene in range(scenes_done, num_scenes):
                ...
                for i_trajectory in range(trajs_done, num_trajectories):
                    ...
                    while self.state != FSMState.EXIT:
                        ...
                        if self.state == FSMState.PLAN_TRAJECTORY:
                            ...
                            if trajectories_.shape[0] < 1:
                                ...
                                # self.state = FSMState.EXECUTE_APPROACH_PATH
                                self.state = FSMState.PLAN_TRAJECTORY # plan again
                                # Skip this scene
                                break
                                # continue
                        ...
                        elif self.state == FSMState.EXECUTE_APPROACH_PATH:
                            current_joints = self.robot.get_current_joint_values()
                            ...
                            if not approach_success:
                                ...
                                if T_6_0_collision is not None:
                                    if CORRECT_ON_TOUCH:
                                        self.correct_touch_model()
                                    # self.state = FSMState.PLAN_TRAJECTORY if not IS_OFFLINE else "EXIT"
                                    self.state = FSMState.PLAN_TRAJECTORY
                                    continue
                            else:
                                self.state = FSMState.EXECUTE_INSERTION_PATH

                        elif self.state == FSMState.EXECUTE_INSERTION_PATH:
                            rospy.loginfo("[FSM] Executing insertion path...")
                            current_joints = self.robot.get_current_joint_values()
                            ...
                            if not success:
                                # self.state = FSMState.EXIT if IS_OFFLINE else "PLAN_TRAJECTORY"
                                self.state = FSMState.PLAN_TRAJECTORY
                            else:
                                rospy.loginfo("[FSM] Insertion path executed successfully.")
                                self.state = FSMState.EXECUTE_OPENING_PATH


                        elif self.state == FSMState.EXECUTE_OPENING_PATH:
                    ...
                    if IS_OFFLINE and self.state == FSMState.EXIT:
