...
class MultiContactForceController:
    ...
    def create_cabinet_model(self, door_params, axis_pos, T_A_W):
        ...
        return Cabinet(
            door_params=np.array([self.door_thickness, width, height, self.static_depth]),
            r=np.array([rx, ry]),
            axis_pos=axis_pos,
            T_A_S=T_A_W,
            save_path=None,
    ...
    def run_single_experiment(self, door_index: int) -> bool:
        ...
        while not rospy.is_shutdown():
            ...
            # -----------------------------------------------------------
            # INSERTION: execute with force monitoring
            # -----------------------------------------------------------
            elif self.state == FSMState.INSERTION:
                
                trajectory = self._planned_trajectory[:3]

                success = self.execute_with_force_monitoring(
                    trajectory,
                ...
                rospy.loginfo("[FSM] Approach successful. Moving to OPENING.")
            ...
            # -----------------------------------------------------------
                ...
                # Use MoveIt to plan a collision-free path to approach configuration (first 3 waypoints)
                path_, success = self.robot.plan_to_joint_goals2(trajectory[:3])
                if not success:
                    rospy.logwarn("[FSM] MoveIt planning failed.")
                    if replan_count < max_replan_attempts:
                        replan_count += 1
                        self.state = FSMState.PLAN
                    continue
                ...
                if not success:
                    rospy.logwarn("[FSM] MoveIt execution failed (force exceeded).")
                    if replan_count < max_replan_attempts:
                        replan_count += 1
                        self.state = FSMState.PLAN
                    continue
