...
if exp_name == 'simulation_exp':
    ...
    cfg_path = os.path.join(pkg_path, 'config/config_multi-c_our_handleless_axis_left_real2_new_table.yaml')
...
# distance criteria
base_size = close_dist_range = 0.35
far_dist_range = 0.9
...
moving_to_static_part_distance = config['cabinet_door_dims']['moving_to_static_part_distance']
distance_from_floor = 0.005 # to ensure that cabinet does not collide with floor

# push_latch_mechanism_length = 0.038 + static_side_width*0.5
# push_latch_mechanism_length = 0.042
push_latch_mechanism_length = config['push_latch_mechanism_length']
# latch_offset = 0.005
latch_offset = 0.0
...
while i < n:
    min_x = cabinet_pose['min_x']
    max_x = cabinet_pose['max_x']
    min_y = cabinet_pose['min_y']
    max_y = cabinet_pose['max_y']
    ...
    if exp_name == 'simulation_exp':
        cabinet_position = [np.random.uniform(min_x, max_x),
        ...
        cabinet_position = [np.random.uniform(min_x, max_x),
                            ...
                            cabinet_door_dims['max_height']*0.5 + moving_to_static_part_distance + T_B_S[2, 3] + static_side_width] # half of door height + robot offset from bottom + offset from bottom
    ...
    if exp_name == 'real_exp':
        ...
        cabinet_model.moving_to_static_part_distance = moving_to_static_part_distance

        # Corner points of the cabinet
        T_pt1_A = np.eye(4)
        T_pt1_A[:3, 3] = np.array([-cabinet_model.d_door, axis_distance + moving_to_static_part_distance + static_side_width, 0.])
        T_pt2_A = np.eye(4)
        T_pt2_A[:3, 3] = np.array([cabinet_model.static_d - cabinet_model.d_door, axis_distance + moving_to_static_part_distance + static_side_width, 0.])
        T_pt3_A = np.eye(4)
        T_pt3_A[:3, 3] = np.array([cabinet_model.static_d - cabinet_model.d_door, -(cabinet_model.w_door - axis_distance + moving_to_static_part_distance + static_side_width), 0.])
        T_pt4_A = np.eye(4)
        T_pt4_A[:3, 3] = np.array([-cabinet_model.d_door, -(cabinet_model.w_door - axis_distance + moving_to_static_part_distance + static_side_width), 0.])
        ...
        angle_deg = axis_pos * np.rad2deg(np.arcsin(push_latch_mechanism_length/(cabinet_model.w_door - axis_distance - latch_offset)))
        pts_x = np.abs(np.array([T_pt1_S[0, 3], T_pt2_S[0, 3], T_pt3_S[0, 3], T_pt4_S[0, 3]]))
        pts_y = np.abs(np.array([T_pt1_S[1, 3], T_pt2_S[1, 3], T_pt3_S[1, 3], T_pt4_S[1, 3]]))
        pts_crit = np.all(pts_x < table_width*0.5) and np.all(pts_y < table_length) and np.all(pts_y > base_size)
